...
export default function AboutPage() {
    ...
    return (
        <main className="min-h-screen bg-black text-white relative pt-24 pb-20 overflow-hidden">
            {/* Background Image / Statue - Positioned to the right */}
            <div className="absolute top-0 right-0 w-full h-full md:w-3/5 lg:w-1/2 pointer-events-none z-0">
                <Image
            ...
            <div className="relative z-10 max-w-screen-2xl mx-auto px-6 md:px-12 lg:px-24">
                ...
                <div className="max-w-3xl">
                    <p className="text-[#780606] text-xs tracking-[0.4em] font-bold uppercase mb-8">
                        ABOUT US
                    </p>

                    <h1 className="font-cinzel text-5xl md:text-7xl lg:text-[5.5rem] font-black leading-[0.9] tracking-tighter mb-12 relative">
                        DEVIL INSIDE<br />
                    ...
                    <div className="space-y-6 text-gray-400 font-grotesk text-[14px] leading-relaxed max-w-xl">
                        <p className="text-[#780606] font-bold tracking-widest text-[10px]">
                        ...
                        <p className="text-[#780606] italic">
                ...
                {/* Features Grid */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mt-24 pt-12 border-t border-white/10">
                ...
                {/* Stats Section */}
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 mt-32 pt-12 border-t border-white/10">
                    {ABOUT_STATS.map((s, i) => (
                        <Stat key={i} {...s} />
                    ))}
                </div>
...
function Feature({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
    return (
        <div className="space-y-4">
            <div className="text-[#780606] w-8 h-8">
                {icon}
            </div>
            <h3 className="font-barlow text-white font-bold tracking-widest text-base uppercase">
                {title}
            </h3>
            <p className="font-grotesk text-gray-500 text-[11px] leading-relaxed">
                {desc}
            </p>
        </div>
    );
...
function Stat({ number, label, sublabel }: { number: string, label: string, sublabel: string }) {
    return (
        <div className="space-y-1">
            <h2 className={`font-cinzel text-4xl md:text-5xl font-black text-[#780606]`}>
                {number}
            </h2>
            <p className="font-bebas text-white tracking-widest text-base">
                {label}
            </p>
            <p className="font-barlow text-gray-600 text-[10px] tracking-[0.2em] uppercase font-bold">
                {sublabel}
            </p>
        </div>
    );
...
function CrownIcon() {
    return (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M2 20h20" />
            <path d="M5 20V8l4 4 3-6 3 6 4-4v12" />
        </svg>
    );
}
