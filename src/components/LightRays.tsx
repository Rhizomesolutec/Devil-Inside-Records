...
const LightRays: React.FC<LightRaysProps> = ({
    ...
    // Intersection observer — just toggles a ref, no re-init
    useEffect(() => {
        if (!containerRef.current) return;
        observerRef.current = new IntersectionObserver(
            (entries) => {
                isVisibleRef.current = entries[0].isIntersecting;
                // Resume loop if coming into view
                if (entries[0].isIntersecting && !animationIdRef.current && meshRef.current && rendererRef.current) {
                    const loop = (t: number) => {
    ...
    useEffect(() => {
        ...
        const initializeWebGL = async () => {
            ...
            // Fade in after a small delay so first frame renders before appearing
            setTimeout(() => { gl.canvas.style.opacity = "1"; }, 120);
...
void main() {
  ...
  gl_FragColor = color;
        ...
        initializeWebGL().then((fn) => { cleanupFn = fn; });

        return () => { cleanupFn?.(); };
        // Only run once on mount — props changes handled by next effect
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);
